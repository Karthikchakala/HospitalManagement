...
export default function AdminDoctorManagementPage() {
    ...
    const fetchDoctors = useCallback(async () => {
        ...
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/admin/doctors`, {
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/admin/doctors/${editingId}`, {
                name: editForm.name,
                                ...
                                <table className="min-w-full">
                                    ...
                                    <tbody>
                                        {doctors.map((doctor) => (
                                            <tr
                                                key={doctor.id}
                                                className={`border-b border-slate-700/50 transition-colors duration-200 ${editingId === doctor.id
                                                    ? 'bg-cyan-500/20'
