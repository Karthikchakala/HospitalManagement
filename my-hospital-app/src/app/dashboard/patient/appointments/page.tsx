...
function AppointmentsContent() {
      ...
      const fetchDoctors = async () => {
        ...
        try {
          const response = await axios.get(
            `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/patient/appointments/doctors/${selectedDepartment}`,
  ...
  useEffect(() => {
    if (selectedDoctor && selectedDate) {
      const fetchTimeSlots = async () => {
        ...
        try {
          const response = await axios.get(
            `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/patient/appointments/times/${selectedDoctor}/${selectedDate}`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
  ...
  const cancelAppointment = async (it: UpcomingAppointment) => {
    ...
    try {
      await axios.patch(
        `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/patient/appointments/${it.type}/${it.appointment_id}/cancel`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const fetchSlotsFor = async (doctorId: number, date: string) => {
    ...
    try {
      const resp = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/patient/appointments/times/${doctorId}/${date}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const submitReschedule = async (e: React.FormEvent) => {
    ...
    try {
      await axios.patch(
        `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/patient/appointments/${resItem.type}/${resItem.appointment_id}/reschedule`,
  ...
  const fetchUpcoming = async () => {
    ...
    try {
      ...
      const resp = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/patient/appointments/upcoming`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const finalizeAppointmentAndBill = async (
    ...
    try {
      await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/patient/appointments`,
        {
      ...
      try {
        await axios.post(
          `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/video/invite`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const txnResponse = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/patient/payment/generate-txn`,
        { amount: price },
        { headers: { Authorization: `Bearer ${token}` } }
      );
