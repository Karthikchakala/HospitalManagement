...
export default function OutpatientsPage() {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      const { data } = await axios.get<OutpatientRow[]>(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/patient/outpatients?${params.toString()}`);
      setList(data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const onBook = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/patient/outpatients/book`, payload);
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 p-6">
      <div className="mx-auto max-w-6xl">
        <div className="mb-6 flex items-center justify-between">
          <h1 className="text-3xl font-bold text-cyan-200">Outpatients</h1>
