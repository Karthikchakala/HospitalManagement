...
export default function StaffOutpatientsPage() {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      const { data } = await axios.get<OutpatientRow[]>(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/staff/outpatients?${params.toString()}`);
      setList(data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const onBook = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/staff/outpatients/book`, payload);
