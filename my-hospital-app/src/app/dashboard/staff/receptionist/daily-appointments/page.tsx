...
export default function ReceptionistDailyAppointmentsPage() {
    ...
    const fetchSchedule = async (opts?: { keepLoading?: boolean }) => {
        ...
        try {
            ...
            const resp = await axios.get('http://localhost:5000/api/staff/appointments/today', {
                headers: { Authorization: `Bearer ${token}` },
                params,
            });
    ...
    const onChangeStatus = async (id: number, nextStatus: string) => {
        ...
        try {
            await axios.patch(`http://localhost:5000/api/staff/appointments/${id}/status`, { status: nextStatus }, {
                headers: { Authorization: `Bearer ${token}` }
            });
