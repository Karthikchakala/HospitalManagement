...
export default function StaffInpatientsPage() {
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        ...
        const normalizedDoctors: Doctor[] = (docData || []).map((d: Record<string, unknown>) => ({
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      const { data } = await axios.get<InpatientRow[]>(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/staff/inpatients?${params.toString()}`);
      setList(data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const fetchListCallback = useCallback(fetchList, [filters]);

  useEffect(() => { fetchListCallback(); }, [fetchListCallback]);
  ...
  const onAdmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/staff/inpatients/admit`, payload);
  ...
  const discharge = async (id: number) => {
    try {
      await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/api/staff/inpatients/discharge/${id}`);
      await fetchList();
  ...
  const docLabel = (d: Doctor) => d.name || `Doctor ${d.doctor_id}`;
